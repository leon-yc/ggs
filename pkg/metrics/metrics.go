...
func Init() error {
	...
	if err := enableProviderMetrics(); err != nil {
		return err
	}
	...
	if archaius.GetBool("ggs.metrics.autometrics.enabled", false) {
		if err := enableAutoRegistryMetrics(); err != nil {
			qlog.Error(err)
		}
	}

	if !archaius.GetBool("ggs.metrics.disableRedisMetrics", false) {
		if err := enableRedisMetrics(); err != nil {
			qlog.Errorf("init redis metrics failed, err:", err.Error())
		}
	}

	if !archaius.GetBool("ggs.metrics.disableErrorLogMetrics", false) {
...
//GetSystemPrometheusRegistry return prometheus registry which ggs use
