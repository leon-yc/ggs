...
func SaveSessionIDFromContext(ctx context.Context, ep string, autoTimeout int) context.Context {
	...
	if err != nil {
		qlog.WithError(err).Warn("session id generate fail, it is impossible")
	}
...
func SaveSessionIDFromHTTP(ep string, autoTimeout int, resp *http.Response, req *http.Request) {
	...
	valueGgsLb := GetSessionFromResp(common.LBSessionID, resp)
	//if session is in resp, then just save it
	if valueGgsLb != "" {
		Save(valueGgsLb, ep, timeValue)
...
func DeletingKeySuccessiveFailure(resp *http.Response) {
	Cache.DeleteExpired()
	if resp == nil {
		valueGgsLb := getLBCookie(common.LBSessionID)
		if valueGgsLb != "" {
			cookieKey := strings.Split(valueGgsLb, "=")
			if len(cookieKey) > 1 {
				Delete(cookieKey[1])
...
func GetSessionID(namespace string) string {
	...
	if !ok || value == nil {
		qlog.Warn("not sessionID in cache")
		return ""
	}
	s, ok := value.(string)
	if !ok {
		qlog.Warn("get sessionID from cache failed")
		return ""
	}
	return s
}
