...
func removeExceededFiles(path string, baseFileName string,
	...
	if err != nil {
		qlog.Errorf("filepath.Walk() path: %s failed: %s", EscapPath(path), err)
		return
	}
	...
	for len(fileList) > maxKeptCount {
		...
		if err != nil {
			qlog.Errorf("remove filePath: %s failed: %s", EscapPath(filePath), err)
			break
		}
...
func compressFile(filePath, fileBaseName string, replaceTimestamp bool) error {
	...
	defer func() {
		err := zipFile.Close()
		if err != nil {
			qlog.Error("can not close log zip file: " + err.Error())
		}
	}()
...
func shouldRollover(fPath string, MaxFileSize int) bool {
	...
	if err != nil {
		qlog.Errorf("state path: %s failed: %s", EscapPath(fPath), err)
		return false
	}
...
func doRollover(fPath string, MaxFileSize int, MaxBackupCount int) {
	...
	if err != nil {
		qlog.Errorf("copy path: %s failed: %s", EscapPath(fPath), err)
	}
	...
	if err != nil {
		qlog.Errorf("truncate path: %s failed: %s", EscapPath(fPath), err)
		return
	}
	f.Close()
...
func doBackup(fPath string, MaxBackupCount int) {
	...
	if err != nil {
		qlog.Errorf("walk path: %s failed: %s", EscapPath(fPath), err)
		return
	}
	...
	for _, file := range rotateFileList {
		...
		if err != nil {
			qlog.Errorf("compress path: %s failed: %s", EscapPath(file), err)
			continue
		}
		err = removeFile(file)
		if err != nil {
			qlog.Errorf("remove path %s failed: %s", EscapPath(file), err)
		}
	}
...
func logRotateFile(file string, MaxFileSize int, MaxBackupCount int) {
	defer func() {
		if e := recover(); e != nil {
			qlog.Errorf("LogRotate file path: %s catch an exception.", EscapPath(file))
		}
	}()
...
func LogRotate(path string, MaxFileSize int, MaxBackupCount int) {
	...
	defer func() {
		if e := recover(); e != nil {
			qlog.Errorf("LogRotate catch an exception")
		}
	}()
	...
	fileList, err := FilterFileList(path, pat)
	if err != nil {
		qlog.Errorf("filepath.Walk() path: %s failed: %s", path, err)
		return
	}
...
func initLogRotate(logFilePath string, option *Options) {
	...
	if option.LogRotateSize <= 0 || option.LogRotateSize > 100 {
