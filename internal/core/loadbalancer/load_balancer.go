...
func Enable(strategyName string) error {
	qlog.Trace("Enable LoadBalancing")
	...
	if strategyName == "" {
		qlog.Info("Empty strategy configuration, use RoundRobin as default")
		return nil
	}
	qlog.Info("Strategy is " + strategyName)
