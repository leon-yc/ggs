...
func (s *HeartbeatService) AddTask(microServiceID, microServiceInstanceID string) {
	...
	qlog.Infof("Add HB task, task:%s", key)
	s.mux.Lock()
...
func (s *HeartbeatService) DoHeartBeat(microServiceID, microServiceInstanceID string) {
	...
	if err != nil {
		qlog.Errorf("Run Heartbeat fail: %s", err)
...
func (s *HeartbeatService) RetryRegister(sid, iid string) {
	for {
		qlog.Info("try to re-register")
		_, err := DefaultServiceDiscoveryService.GetAllMicroServices()
		if err != nil {
			qlog.Errorf("DefaultRegistrator is not healthy %s", err)
			continue
		}
	...
	qlog.Warn("Re-register self success")
}
...
func (s *HeartbeatService) ReRegisterSelfMSandMSI() error {
	...
	if err != nil {
		qlog.Errorf("The reRegisterSelfMSandMSI() startMicroservice failed: %s", err)
		return err
	}

	err = RegisterMicroserviceInstances()
	if err != nil {
		qlog.Errorf("The reRegisterSelfMSandMSI() startInstances failed: %s", err)
		return err
	}
	return nil
}
...
func reRegisterSelfMSI(sid, iid string) error {
	...
	if err != nil {
		qlog.Errorf("RegisterInstance failed: %s", err)
		return err
	}

	value, ok := SelfInstancesCache.Get(microServiceInstance.ServiceID)
	if !ok {
		qlog.Warnf("RegisterMicroServiceInstance get SelfInstancesCache failed, microServiceID/instanceID: %s/%s", sid, instanceID)
	}
	instanceIDs, ok := value.([]string)
	if !ok {
		qlog.Warnf("RegisterMicroServiceInstance type asserts failed, microServiceID/instanceID: %s/%s", sid, instanceID)
	}
	var isRepeat bool
