...
import (
	...
	"github.com/leon-yc/ggs/third_party/forked/k8s.io/apimachinery/pkg/util/sets"
)
...
func (c *CacheManager) AutoSync() {
	...
	if config.GetServiceDiscoveryWatch() {
		...
		if err != nil {
			qlog.Errorf("Watch failed. Self Micro service Id:%s. %s", runtime.ServiceID, err)
		}
		qlog.Tracef("Watching Instances change events.")
	}
	...
	if refreshInterval == "" {
		...
		if err != nil {
			qlog.Errorf("refeshInterval is invalid. So use Default value, err %s", err)
...
func (c *CacheManager) refreshCache() {
	if archaius.GetBool("ggs.service.registry.autodiscovery", false) {
		err := c.registryClient.SyncEndpoints()
		if err != nil {
			qlog.Errorf("get sc endpoints failed: %s", err)
		}
	}
	err := c.pullMicroServiceInstance()
	if err != nil {
		qlog.Errorf("AutoUpdateMicroserviceInstance failed: %s", err)
		//connection with sc may lost, reset the revision
		c.registryClient.ResetRevision()
	}

	if archaius.GetBool("ggs.service.registry.autoSchemaIndex", false) {
		err = c.MakeSchemaIndex()
		if err != nil {
			qlog.Errorf("MakeSchemaIndex failed: %s", err)
		}
	}

	if archaius.GetBool("ggs.service.registry.autoIPIndex", false) {
		err = c.MakeIPIndex()
		if err != nil {
			qlog.Errorf("Auto Update IP index failed: %s", err)
		}
	}
	...
	for _, service := range services {
		for _, inst := range service.Instances {
			for _, uri := range inst.Endpoints {
				u, err := url.Parse(uri)
				if err != nil {
					qlog.Errorf("Wrong URI %s: %s", uri, err)
					continue
				}
		...
		for _, schemaName := range ms.Schemas {
			...
			if interfaceName != "" {
				...
				if !ok {
					...
					qlog.Tracef("New Interface added in the Index Cache : %s", interfaceName)
				} else {
				...
				if !ok {
					...
					qlog.Tracef("New Service added in the Index Cache : %s", serviceID)
				} else {
...
func (c *CacheManager) pullMicroServiceInstance() error {
	...
	if err != nil {
		if err == client.ErrNotModified || err == client.ErrEmptyCriteria {
			qlog.Trace(err.Error())
		} else {
			qlog.Error("Refresh local instance cache failed: " + err.Error())
		}
	}
	filter(instances)
...
func getServiceSet(exist []*proto.FindService) (sets.String, map[string]sets.String) {
	...
	for _, service := range exist {
		if service == nil {
			qlog.Warn("FindService info is empty")
			continue
		}
		if service.Service == nil {
			qlog.Warn("provider info is empty")
			continue
		}
...
func filter(providerInstances map[string][]*proto.MicroServiceInstance) {
	...
	for serviceName, instances := range providerInstances {
		...
		for _, ins := range instances {
			switch {
			case ins.Version == "":
				qlog.Warn("do not support old service center, plz upgrade")
				continue
			case ins.Status != common.DefaultStatus && ins.Status != common.TESTINGStatus:
				downs[ins.InstanceId] = struct{}{}
				qlog.Tracef("do not cache the instance in '%s' status, instanceId = %s/%s",
...
func watch(response *client.MicroServiceInstanceChangedEvent) {
	...
	case client.EventError:
		qlog.Warnf("MicroServiceInstanceChangedEvent action is error, MicroServiceInstanceChangedEvent = %s", response)
		break
	default:
		qlog.Warnf("Do not support this Action = %s", response.Action)
		return
	}
}
...
func createAction(response *client.MicroServiceInstanceChangedEvent) {
	...
	if !ok {
		qlog.Errorf("ServiceID does not exist in MicroServiceInstanceCache,action is EVT_CREATE.key = %s", key)
		return
	}
	if response.Instance.Status != client.MSInstanceUP {
		qlog.Warnf("createAction failed,MicroServiceInstance status is not MSI_UP,MicroServiceInstanceChangedEvent = %s", response)
		return
	}
	...
	qlog.Tracef("Cached Instances,action is EVT_CREATE, sid = %s, instances length = %d", response.Instance.ServiceId, len(microServiceInstances))
}

// deleteAction delete micro-service instance
func deleteAction(response *client.MicroServiceInstanceChangedEvent) {
	key := response.Key.ServiceName
	qlog.Tracef("Received event EVT_DELETE, sid = %s, endpoints = %s", response.Instance.ServiceId, response.Instance.Endpoints)
	if err := registry.HealthCheck(key, response.Key.Version, response.Key.AppID, ToMicroServiceInstance(response.Instance)); err == nil {
		return
	}
	microServiceInstances, ok := registry.MicroserviceInstanceIndex.Get(key, nil)
	if !ok {
		qlog.Errorf("ServiceID does not exist in MicroserviceInstanceCache, action is EVT_DELETE, key = %s", key)
		return
	}
	...
	qlog.Tracef("Cached [%d] Instances of service [%s]", len(newInstances), key)
}
...
func updateAction(response *client.MicroServiceInstanceChangedEvent) {
	...
	microServiceInstances, ok := registry.MicroserviceInstanceIndex.Get(key, nil)
	if !ok {
		qlog.Errorf("ServiceID does not exist in MicroserviceInstanceCache, action is EVT_UPDATE, sid = %s", key)
		return
	}
	if response.Instance.Status != client.MSInstanceUP {
		qlog.Warnf("updateAction failed, MicroServiceInstance status is not MSI_UP, MicroServiceInstanceChangedEvent = %s", response)
		return
	}
	msi := ToMicroServiceInstance(response.Instance).WithAppID(response.Key.AppID)
	...
	case InstanceIDIsNotExist:
		...
		break
	default:
		qlog.Warnf("updateAction error, iid:%s", response.Instance.InstanceId)
	}
	registry.MicroserviceInstanceIndex.Set(key, microServiceInstances)
	qlog.Tracef("Cached Instances,action is EVT_UPDATE, sid = %s, instances length = %d", response.Instance.ServiceId, len(microServiceInstances))
}
