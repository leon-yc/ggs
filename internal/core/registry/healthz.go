...
import (
	...
	ggscli "github.com/leon-yc/ggs/internal/core/client"
	"github.com/leon-yc/ggs/internal/core/config"
	"github.com/leon-yc/ggs/internal/healthz/client"
...
func (hc *HealthChecker) check() {
	for pack := range hc.delCh {
		...
		for _, r := range rs {
			cr := <-r
			if cr.Err != nil {
				qlog.Tracef("Health check instance %s failed, %s",
					cr.Item.ServiceKey(), cr.Err)
				hc.removeFromCache(cr.Item)
				continue
			}
			qlog.Tracef("Health check instance %s %s is still alive, keep it in simpleCache",
...
func (hc *HealthChecker) removeFromCache(i *WrapInstance) {
	...
	qlog.Tracef("Health check: cached [%d] Instances of service [%s]", len(is), i.ServiceName)
}
...
func RefreshCache(service string, ups []*MicroServiceInstance, downs map[string]struct{}) {
	...
	for _, exp := range mapExps {
		...
		if _, ok := mapUps[exp.InstanceID]; ok {
			...
			qlog.Trace(fmt.Sprintf("cache instance: %v", exp))
			continue
		} else {
			for p, ep := range exp.EndpointsMap {
				if err := ggscli.Close(p, service, ep); err != nil {
					if err != ggscli.ErrClientNotExist {
						qlog.Error(fmt.Sprintf("can not close [%s] client for service [%s],intance [%s,%s,%s]: %s",
							p, service, exp.InstanceID, ep, exp.HostName, err))
					}
				} else {
					qlog.Trace(fmt.Sprintf("closed [%s] client for service [%s],intance [%s,%s,%s]",
	...
	qlog.Tracef("Cached [%d] Instances of service [%s]", len(lefts), service)
}
