...
const (
	DefaultRegistratorPlugin       = "consul"
	DefaultServiceDiscoveryPlugin  = "consul"
	DefaultContractDiscoveryPlugin = "consul"
	Name                           = "registry"
...
func enableRegistrator(opts Options) error {
	...
	if err := RegisterMicroservice(); err != nil {
		qlog.Errorf("start backoff for register microservice: %s", err)
		startBackOff(RegisterMicroservice)
	}
	go HBService.Start()

	qlog.Infof("Enable [%s] registrator.", rt)
	return nil
}
...
func InstallRegistrator(name string, f func(opts Options) Registrator) {
	registryFunc[name] = f
	qlog.Trace("Installed registry plugin: " + name)
}
	...
	default:
		{
			...
			qlog.Error(tmpErr.Error())
			return tmpErr
		}
	}
	if isAutoRegister {
		if err := RegisterMicroserviceInstances(); err != nil {
			qlog.Errorf("start back off for register microservice instances background: %s", err)
