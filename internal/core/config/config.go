...
func populateServiceRegistryAddress() {
	...
	registryAddrFromEnv := readCseAddress(common.EnvGGSSCEndpoint, common.CseRegistryAddress)
	if registryAddrFromEnv != "" {
		qlog.WithField("ep", registryAddrFromEnv).Debug("detect env")
		GlobalDefinition.Ggs.Service.Registry.Registrator.Address = registryAddrFromEnv
		GlobalDefinition.Ggs.Service.Registry.ServiceDiscovery.Address = registryAddrFromEnv
		GlobalDefinition.Ggs.Service.Registry.ContractDiscovery.Address = registryAddrFromEnv
		GlobalDefinition.Ggs.Service.Registry.Address = registryAddrFromEnv
	}
}
...
func populateConfigCenterAddress() {
	...
	configCenterAddrFromEnv := readCseAddress(common.EnvGGSCCEndpoint, common.CseConfigCenterAddress)
	if configCenterAddrFromEnv != "" {
		GlobalDefinition.Ggs.Config.Client.ServerURI = configCenterAddrFromEnv
	}
}
...
func readCseAddress(firstEnv, singleEnv string) string {
	...
	if addrFromEnv == "" {
		addrFromEnv = os.Getenv(common.EnvGGSEndpoint)
		if addrFromEnv == "" {
...
func populateMonitorServerAddress() {
	...
	if monitorServerAddrFromEnv != "" {
		GlobalDefinition.Ggs.Monitor.Client.ServerURI = monitorServerAddrFromEnv
	}
}
...
func populateVersion() {
	if e := archaius.GetString(common.Version, common.DefaultVersion); e != "" {
...
func populateTenant() {
	if GlobalDefinition.Ggs.Service.Registry.Tenant == "" {
		GlobalDefinition.Ggs.Service.Registry.Tenant = common.DefaultApp
	}
}
...
func readMicroServiceFromArchaius() error {
	...
	err := archaius.UnmarshalConfig(&microserviceCnf)
	if err != nil {
		return err
	}
...
func Init() error {
	...
	qlog.Info("archaius init success")
	...
	qlog.Info("Host name is " + runtime.HostName)
	return err
}
