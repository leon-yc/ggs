...
import (
	...
	bo "github.com/leon-yc/ggs/internal/pkg/backoff"
...
const (
	...
	EnvCheckSCIInterval = "GGS_SC_HEALTH_CHECK_INTERVAL"
)
...
func (c *RegistryClient) Initialize(opt Options) (err error) {
	...
	//Set the API Version based on the value set in chassis.yaml ggs.service.registry.api.version
...
func (c *RegistryClient) SyncEndpoints() error {
	...
	if len(eps) != 0 {
		...
		qlog.Info("Sync service center endpoints " + strings.Join(eps, ","))
		return nil
	}
...
func (c *RegistryClient) GetDefaultHeaders() http.Header {
	...
	headers := http.Header{
		...
		HeaderUserAgent:   []string{"ggs-serviceregistry-client/1.0.0"},
...
func (c *RegistryClient) FindMicroServiceInstances(consumerID, appID, microServiceName,
	...
	if resp.StatusCode >= 200 && resp.StatusCode < 300 {
		...
		if r != c.revision && r != "" {
			c.revision = r
			qlog.Trace("Instance got new revision " + c.revision)
		}
