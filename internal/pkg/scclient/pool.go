...
func (p *AddressPool) SetAddress(addresses []string) {
	...
	qlog.Info("set service center endpoints " + strings.Join(addresses, ","))
...
func (p *AddressPool) GetAvailableAddress() string {
	...
	addr, err := next()
	if err != nil {
		qlog.Warn("use default service center address")
		return DefaultAddr
	}
	return addr
}
...
func (p *AddressPool) checkConnectivity() {
	...
	for _, v := range p.addressMap {
		...
		if err != nil {
			qlog.WithError(err).Error("can not connect to sc endpoint")
